...
export class UpdateAgentStatusProcessor extends AbstractHandler {
    ...
    updateCapabilities = (request: Request) => {
        ...
        if (request.data.role === 'IA' && request.data.status === '1') this.addCapabilities(request.projectDid, [request.data.agentDid], 'SubmitClaim');
        ...
        if (request.data.role === 'IA' && request.data.status === '2') this.removeCapabilities(request.projectDid, request.data.agentDid, 'SubmitClaim');
